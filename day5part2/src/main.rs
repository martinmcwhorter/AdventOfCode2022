...
fn main() {
    ...
    instructions
        ...
        .for_each(|inst| {
            let amount: usize = inst.clone().nth(1).unwrap().parse().unwrap();
            let from: u8 = inst.clone().nth(3).unwrap().parse().unwrap();
            let to: u8 = inst.clone().nth(5).unwrap().parse().unwrap();

            let len = stack_map.get_mut(&from).unwrap().len();
            let mut containers: Vec<_> = stack_map.get_mut(&from).unwrap().drain(len - amount..len).collect();
            stack_map.get_mut(&to).unwrap().append(&mut containers);
        });
